...
def draw(canvas):
    ...
    if PAD_WIDTH + ball_vel[0] -1 <= ball_pos[0] - BALL_RADIUS <= (PAD_WIDTH - 1) and ball_vel[0] < 0:
    ...
    if WIDTH + ball_vel[0] >= ball_pos[0] + BALL_RADIUS >= WIDTH - PAD_WIDTH - 1 and ball_vel[0] > 0:
    ...
    and (BALL_RADIUS - 1) <= ball_pos[0] <= (WIDTH - BALL_RADIUS - 1)\
    and ball_vel[1] > 0:
    ...
    and ball_vel[1] < 0:
        ball_vel[1] = - ball_vel[1]
    # draw scores
    canvas.draw_text(str(score1), (149, 100), 40, 'Red')
    canvas.draw_text(str(score2), (449, 100), 40, 'Blue')
       
def keydown(key):
    global paddle1_vel, paddle2_vel
    if key == simplegui.KEY_MAP['w']:
        paddle1_vel -= paddle_vel_acc
    if key == simplegui.KEY_MAP['s']:
        paddle1_vel += paddle_vel_acc
    if key == simplegui.KEY_MAP['up']:
        paddle2_vel -= paddle_vel_acc
